...
export default function BlogPage() {
  ...
  async function generate() {
    ...
    try {
      const res = await fetch("/api/blog", {
        ...
        body: JSON.stringify({ topic, keyword, length, brand }),
      });
      const data = await res.json();
      if (data.post) {
        setPost(data.post);
        if (data.source === "fallback") setNote("Preview draft (no API key set — add ANTHROPIC_API_KEY for full AI posts).");
  ...
  function copyHtml() {
    ...
    try {
      ...
      setNote("HTML copied ✓ — paste into your website editor.");
    } catch {
      setNote("Select the HTML and copy manually.");
    }
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-10">
      <span className="eyebrow">Blog writer</span>
      <h1 className="mt-2 font-display text-3xl font-extrabold tracking-tight">AI blogs that help you rank</h1>
      <p className="mt-2 text-ink2">Turn a topic into a ready-to-publish, SEO-optimized post in your brand voice.</p>
      ...
      <div className="card mt-6">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          <div>
            ...
            <input className="inp" placeholder="best coffee beans Austin" value={keyword} onChange={(e) => setKeyword(e.target.value)} />
          </div>
          <div>
            <label className="label">Length</label>
            <select className="inp" value={length} onChange={(e) => setLength(e.target.value as typeof length)}>
              <option value="short">Short (~500 words)</option>
              <option value="medium">Medium (~850 words)</option>
              <option value="long">Long (~1400 words)</option>
            </select>
          </div>
        </div>
        <div className="mt-4">
          <button className="btn-primary" onClick={generate} disabled={busy || !ready}>{busy ? "Writing…" : "Generate post"}</button>
        </div>
        ...
        <div className="card mt-6">
          ...
          {post.keywords?.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2">
              {post.keywords.map((k) => (<span key={k} className="rounded-md border border-line bg-surface2 px-2 py-1 font-mono text-xs text-ink2">{k}</span>))}
            </div>
          )}
          <article className="prose-unbranded mt-5" dangerouslySetInnerHTML={{ __html: post.html }} />
          <div className="mt-6 rounded-xl border border-line bg-surface2 px-4 py-3 text-sm text-ink2">
            <b className="text-ink">Publish:</b> copy the HTML into WordPress/Shopify/Webflow, or download it. One-click publishing straight to your site arrives in Phase 2.
          </div>
        </div>
      )}
    </div>
