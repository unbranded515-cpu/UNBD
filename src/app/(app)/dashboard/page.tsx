...
export default function Home() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-10">
      ...
      {a?.aeo && a.aeo.length > 0 && (
        <div className="card mt-4">
          <div className="mb-3 flex items-center gap-2">
            <span className="grid h-8 w-8 place-items-center rounded-lg bg-surface2 text-coral">
              <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="1.7"><circle cx="12" cy="12" r="9" /><path d="M3 12h18M12 3a15 15 0 0 1 0 18M12 3a15 15 0 0 0 0 18" /></svg>
            </span>
            <h3 className="font-display text-lg font-extrabold">AI visibility</h3>
            <span className="rounded-md bg-coral/12 px-2 py-0.5 font-mono text-[10px] font-bold text-coralink">ChatGPT · Claude · Gemini</span>
          </div>
          <p className="mb-3 text-sm text-ink2">What to fix so AI assistants understand and recommend {active.businessName}:</p>
          <div className="divide-y divide-line">
            {a.aeo.slice(0, 4).map((c, i) => (
              <div key={i} className="flex items-start gap-3 py-2.5">
                <span className={`mt-0.5 flex-none rounded px-2 py-0.5 text-[11px] font-bold uppercase ${c.state === "good" ? "bg-good/15 text-good" : c.state === "warn" ? "bg-warn/18 text-warn" : "bg-bad/15 text-bad"}`}>{c.state === "good" ? "Good" : "Fix"}</span>
                <div><b className="text-sm">{c.title}</b><p className="text-sm text-muted">{c.detail}</p></div>
              </div>
            ))}
          </div>
          <Link href="/seo" className="mt-3 inline-block text-sm font-bold text-coralink hover:underline">See all AI + SEO fixes →</Link>
        </div>
      )}
