...
export default function SeoPage() {
  ...
  // Load this brand's saved data whenever the active brand changes.
  useEffect(() => {
    setUrl(active?.website || "");
    setKeyword(active?.keywords?.[0] || "");
    setResult(active?.audit ? { url: active.website, ...active.audit } : null);
    setError("");
  ...
  async function run() {
    ...
    try {
      const res = await fetch("/api/audit", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ url, keyword }) });
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-10">
      <span className="eyebrow">SEO &amp; Ranking{active ? ` · ${active.businessName}` : ""}</span>
      <h1 className="mt-2 font-display text-3xl font-extrabold tracking-tight">Website ranking audit</h1>
      <p className="mt-2 text-ink2">The on-page signals Google uses to rank local businesses — with the exact fixes.</p>

      <div className="card mt-6">
        <div className="grid gap-3 sm:grid-cols-2">
          <div><label className="label">Website URL</label><input className="inp" placeholder="yourbusiness.com" value={url} onChange={(e) => setUrl(e.target.value)} onKeyDown={(e) => e.key === "Enter" && run()} /></div>
          <div><label className="label">Target keyword (optional)</label><input className="inp" placeholder="best coffee in Austin" value={keyword} onChange={(e) => setKeyword(e.target.value)} onKeyDown={(e) => e.key === "Enter" && run()} /></div>
        </div>
        <div className="mt-4"><button className="btn-primary" onClick={run} disabled={busy}>{busy ? "Scanning…" : result ? "Re-run audit" : "Run audit"}</button></div>
      ...
      {result && (
        <div className="card mt-6">
          <div className="flex flex-wrap items-center gap-5">
            <div className="grid h-24 w-24 flex-none place-items-center rounded-full" style={{ background: `conic-gradient(${ringColor} ${result.score}%, #FFF4EC 0)` }}>
              <div className="grid h-[76px] w-[76px] place-items-center rounded-full bg-paper"><b className="font-display text-2xl" style={{ color: ringColor }}>{result.score}</b></div>
            </div>
            <div className="min-w-0 flex-1">
              <div className="font-mono text-xs uppercase tracking-wide" style={{ color: ringColor }}>{result.score >= 75 ? "Strong" : result.score >= 50 ? "Needs work" : "At risk"}</div>
